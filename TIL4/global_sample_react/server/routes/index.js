...
// SSL 문제 해결을 위한 간단한 HTTPS 에이전트 설정
const httpsAgent = new https.Agent({
    rejectUnauthorized: false // SSL 인증서 검증 완전 우회
});

// axios 기본 설정에 HTTPS 에이전트 적용
...
router.post('/result', (req, res) => {
    ...
    const responseData = {
        ...
        locale: req.body.locale                           // 사용자 locale 정보
    }

    // Client Page(ReactJS)로 결과전송
    const resultParam = encodeURIComponent(JSON.stringify(responseData));
    res.status(301).redirect(process.env.CLIENT_HOSTNAME+ '/ko/payment/result?result=' + resultParam);
})
