...
/* POST /api/cancel
   국내 결제 취소 API
   페이플 국내 결제 취소 프로세스: 인증 → 환불 URL 받기 → 환불 요청
 */
router.post('/cancel', async (req, res) => {
    try {
        console.log('결제 취소 요청:', req.body);
        
        // 1단계: 취소 인증 (PCD_PAYCANCEL_FLAG: 'Y')
        const authParams = {
            PCD_PAYCANCEL_FLAG: 'Y'
        };
        
        const authResponse = await axios.post(process.env.REACT_APP_AUTH_URL, {
        ...
        console.log('인증 응답:', authResponse.data);
        
        if (authResponse.data.result !== 'success') {
            return res.status(400).json({ 
                error: true, 
                message: '취소 인증 실패: ' + authResponse.data.result_msg 
            });
        }
        ...
        const refundParams = {
            ...
            PCD_AUTH_KEY: authResponse.data.auth_key,
            PCD_REFUND_KEY: process.env.REACT_APP_PCD_REFUND_KEY,
            PCD_PAYCANCEL_FLAG: "Y",
            PCD_PAY_OID: req.body.PCD_PAY_OID,              // 주문번호
            PCD_PAY_DATE: req.body.PCD_PAY_DATE,            // 결제일 (YYYYMMDD)
            PCD_REFUND_TOTAL: req.body.PCD_REFUND_TOTAL,    // 환불 요청금액
            PCD_REFUND_TAXTOTAL: req.body.PCD_REFUND_TAXTOTAL || 0,  // 환불 부가세
            PCD_PAYER_NAME: req.body.PCD_PAYER_NAME,        // 결제자명
            PCD_PAYER_NO: req.body.PCD_PAYER_NO,            // 가맹점 회원번호
            PCD_PAY_GOODS: req.body.PCD_PAY_GOODS,          // 상품명
            PCD_PAYER_EMAIL: req.body.PCD_PAYER_EMAIL       // 결제자 이메일
        };
        
        console.log('환불 요청 파라미터:', refundParams);
        
        const refundResponse = await axios.post(refundURL, JSON.stringify(refundParams), {
            headers: {
                'content-type': 'application/json',
                'referer': process.env.REACT_APP_HOSTNAME
            }
        ...
        console.log('환불 응답:', refundResponse.data);
        ...
        res.status(500).json({
            error: true,
            message: '결제 취소 중 오류가 발생했습니다: ' + (error.response?.data?.result_msg || error.message)
        });
    }
});
